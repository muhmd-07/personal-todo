    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative h-20 w-8 flex items-end">
        ...
        <div
          ...
          style={{ height: `${Math.max(day.completed > 0 ? 40 : 15, 15)}%` }}
        />
      </div>
...
function RefreshButton() {
  ...
  const handleRefresh = useCallback(() => {
    ...
    setState('spinning')
    router.refresh()
    // Show spinning for 900ms, then flash "done" for 600ms
    ...
    <div className="mx-auto max-w-3xl px-6 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <StatCard
          ...
          icon={<svg fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="size-4 text-[var(--color-text-muted)]"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={<svg fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className={`size-4 ${stats.overdueCount > 0 ? 'text-amber-400' : 'text-[var(--color-text-muted)]'}`}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>}
        />
        <StatCard
          label="High priority"
          value={stats.highPriorityCount}
          accent={stats.highPriorityCount > 0 ? 'text-orange-400' : undefined}
          icon={<svg fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className={`size-4 ${stats.highPriorityCount > 0 ? 'text-orange-400' : 'text-[var(--color-text-muted)]'}`}><path strokeLinecap="round" strokeLinejoin="round" d="M3 3l1.664 1.664M21 21l-1.5-1.5m-5.485-1.242L12 17.25 4.5 21V8.742m.164-4.078a2.15 2.15 0 011.743-1.342 48.507 48.507 0 0111.186 0c1.1.128 1.907 1.067 1.907 2.185V19.5M4.664 4.664L19.5 19.5" /></svg>}
        />
        <StatCard
          label="Upcoming (7d)"
        ...
        <StatCard
          ...
          icon={<svg fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="size-4 text-[var(--color-text-muted)]"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zM3.75 12h.007v.008H3.75V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 5.25h.007v.008H3.75v-.008zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" /></svg>}
        />
      </div>
      ...
      {focusTasks.length > 0 && (
        <div>
          ...
          <ul className="space-y-2">
            {focusTasks.map((task) => (
              <li key={task.id} className="flex items-center gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3.5 hover:border-[var(--color-border-strong)] transition">
